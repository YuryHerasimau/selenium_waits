...
def test_register_with_explicit_waits(driver, wait, fake_login, fake_password):
    ...
    ), f"Page title doesn't match. Actual: {actual_title}. Expected: {expected_title}"

    start_test_button = wait.until(
        EC.element_to_be_clickable((By.XPATH, '//*[@id="startTest"]'))
    )
    assert start_test_button.is_displayed()
    expected_text = "Начать тестирование"
    assert (
        start_test_button.text == expected_text
    ), f"Button text doesn't match. Actual: {start_test_button.text}. Expected: {expected_text}"
    ...
    success_message = wait.until(
        EC.element_to_be_clickable((By.XPATH, '//*[@id="successMessage"]'))
    )
    expected_success_message = "Вы успешно зарегистрированы!"
    assert (
        success_message.text == expected_success_message
    ), f"Success message text doesn't match. Actual: {success_message.text}. Expected: {expected_success_message}"
